...
int main()
{
    ...
    Mfull.runSimulation(20000,1,100,0.01,1.,results);
...
Order Market::strategy(int agentID,int day, int period)
{
    ...
    if(pt>marketPrice)
    {
        ...
        O.vol = int( V(rng) );
        // cap volume to one asset
        O.vol = std::min(O.vol,1);
    }
    else
    {
        O.oType = ask;  
            ...
            if( askCurrentBest.price <= Onew.price )
            {
                ...
                if(volumeExecuted>0)
                {
                    ...
                    if(limitOrderBookAsks.size()>0 && limitOrderBookBids.size()>0 )
                    {
                        ...
                        marketPrice = askCurrentBest.price;
                }
            ...
            if( bidCurrentBest.price >= Onew.price )
            {
                ...
                if(volumeExecuted>0)
                {
                    ...
                    if(limitOrderBookAsks.size()>0 && limitOrderBookBids.size()>0 )
                    {
                        ...
                        marketPrice = bidCurrentBest.price;
                }
